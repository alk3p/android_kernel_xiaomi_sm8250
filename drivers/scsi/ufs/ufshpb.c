...
static void ufshpb_pre_req_compl_fn(struct request *req, blk_status_t error)
{
	...
	ufs_spin_lock_irqsave(&hpb->hpb_lock, flags);
	ufshpb_put_pre_req(pre_req->hpb, pre_req);
	ufs_spin_unlock_irqrestore(&hpb->hpb_lock, flags);
					...
					struct request *req)
{
	...
	ufs_spin_lock_irqsave(q->queue_lock, flags);
	...
	ufs_spin_unlock_irqrestore(q->queue_lock, flags);
...
static int ufshpb_map_req_error(struct ufshpb_req *map_req)
{
	...
	if (rgn->rgn_state == HPBREGION_PINNED) {
		...
		ufs_spin_lock_irqsave(&hpb->rsp_list_lock, flags);
		ufshpb_update_inactive_info(hpb, map_req->rb.rgn_idx);
		ufs_spin_unlock_irqrestore(&hpb->rsp_list_lock, flags);
					 ...
					 struct ufshpb_rsp_field *rsp_field)
{
	 ...
	 */
	ufs_spin_lock(&hpb->rsp_list_lock);
	...
	ufs_spin_unlock(&hpb->rsp_list_lock);
