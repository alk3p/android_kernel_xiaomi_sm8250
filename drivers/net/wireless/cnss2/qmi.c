	...
	case CNSS_BDF_ELF:
		if (plat_priv->board_info.board_id == 0xFF) {
			if (hw_platform_ver == HARDWARE_PLATFORM_LMI) {
				...
				if (get_hw_country_version() == (uint32_t)CountryGlobal)
				    snprintf(filename, filename_len, ELF_BDF_FILE_NAME_J3S_GLOBAL);
				else if (get_hw_country_version() == (uint32_t)CountryIndia)
				    snprintf(filename, filename_len, ELF_BDF_FILE_NAME_J3S_INDIA);
				else
				    snprintf(filename, filename_len, ELF_BDF_FILE_NAME_J3S);
			} else {
