	...
	case DSI_BACKLIGHT_DCS:
		if (mi_cfg->fod_backlight_flag) {
			...
			if (mi_cfg->hbm_51_ctrl_flag &&
				...
				(mi_cfg->hbm_enabled && !mi_cfg->thermal_hbm_disabled))) {
				DSI_INFO("fod_hbm_enabled(%d),hbm_enabled(%d),"
					"thermal_hbm_disabled(%d),skip set backlight %d\n",
...
void dsi_panel_put(struct dsi_panel *panel)
{
	...
	wakeup_source_trash(&panel->mi_cfg.aod_wakelock);
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	/* backlight dimming off when fps switch to avoid screen flicker */
